...
def pre_processing_movie (dataDir,stimInputDir): #sebastian's version
    
    stack_path=dataDir+'*stack*.tif'
    stack_path=glob.glob(stack_path)[0]
    # Generate necessary directories for figures
    current_t_series=os.path.basename(dataDir)
    ...
    try: 
        ...
        #movie_path = os.path.join(dataDir, stack_path)
        # movie_path = os.path.join(dataDir, 'motCorr.sima',
                                  # '{t_name}_motCorr.tif'.format(t_name=current_t_series))
        time_series = io.imread(movie_path)
    except IOError:
...
def get_stim_xml_params(t_series_path, stimInputDir): #sebastian's version
    ...
    (stimType, rawStimData) = readStimOut(stimInputDir, 
    ...
    stimulus_information['epoch_dur'] = np.array(epochDur) # Seb: consider to delete this line. Redundancy
    stimulus_information['random'] = np.array(isRandom) # Seb: consider to delete this line. Redundancy
    ...
    stimulus_information['frame_timings'] = np.array(imagetimes)
    stimulus_information['input_data'] = np.array(stimInputData) # Seb: consider to delete this line. Redundancy
...
def get_stim_xml_params(t_series_path, stimOuputFile,original_stimDir,cplusplus=False): #from sebastians code
    """ Gets the required stimulus and imaging parameters.
    ...
    #stimOutPath = os.path.join(t_series_path, '_stimulus_output_*')
    #stimOutFile = (glob.glob(stimOutPath))[0]
    (stimType, rawStimData) = readStimOut(stimOutFile=stimOuputFile, 
...
def separate_trials_video(time_series,stimulus_information,frameRate):
    ...
    for iEpoch in trialCoor:
        ...
        trial_len =  min(trial_lens)-1 # Juan changed this from -4 to -1
...
def separate_trials_ROI_v4(time_series,rois,stimulus_information,frameRate, 
                           ...
                           max_resp_trial_len = 'max',mov_av=False,add_prev_traces=False):
    """ Separates trials epoch-wise into big lists of whole traces, response traces
    and baseline traces. Adds responses and whole traces into the ROI_bg 
    instances.
    ...
    # dF/F calculation
    for iROI, roi in enumerate(rois):
        if add_prev_traces: #juan edit
            roi.appendPrevTraces()  
        #roi.mask=np.where(roi.mask,1,np.nan)
        local_mask=np.where(roi.mask,1,np.nan)
        roi.raw_trace = np.nanmean(time_series*local_mask[np.newaxis,:,:],axis=(1,2))
        roi.calculateDf(method=df_method,moving_avg = mov_av, bins = 3)
        if df_use:
...
def run_analysis(analysis_params, rois,experiment_conditions,
                 ...
                 exp_ID=None,df_method=None,keep_prev=False): #edited by Juan
    """
    asd
    """
    ...
    if analysis_type == 'A_B_steps':
        ...
        rois = ROI_mod.conc_traces(rois, interpolation = True, int_rate = 10,df_method=None)
        ...
        map(lambda roi: roi.calculate_DSI_PD(method='Mazurek'), rois)
...
def select_properties_plot(rois , analysis_type):
    ...
    if analysis_type == 'gratings_transfer_rois_save' or\
        ...
        colormaps = ['viridis','viridis','viridis','viridis','gist_rainbow','gist_rainbow']
...
def run_ROI_selection(extraction_params,stack,image_to_select=None,**kwargs):
    ...
    if extraction_params['type'] == 'manual':
        ...
        (stimulus_information, imaging_information) = \
                    get_stim_xml_params(dataDir, stimPaths[def_ix],input_stim_dir,cplusplus=cplusplus) 
        ...
        roi_masks, all_rois_image, polarity_list, background, metadata = run_cluster_analysis_MH(dataDir,fig_dir,stack,stimulus_information, imaging_information,categories,category_names,I_zone,clustering_mode=clustering_mode)
...
def load_Tseries(dataDir,number_of_cycles,lenghtCycles,file_string='stack*Mcorr.tif'):
    """
...
def import_clusters(Tser,manual=False,FFT_filtered=False):
    ...
    if manual==False:
        ...
        for idx,element in enumerate(clusters):
            ...
            elif 'background' in element.tags:
                BG=np.squeeze(np.array(element))
                #background=np.reshape(np.array(element),(dataset.frame_shape[1],dataset.frame_shape[2],1))
                #background.astype(int)
                BG=BG.astype(bool)
    ...
    data_xDim = dataset.frame_shape[1]
    data_yDim = dataset.frame_shape[2]
    all_clusters_image = np.zeros(shape=(data_xDim,data_yDim))
    all_clusters_image[:] = np.nan
    for index, roi in enumerate(clusters):
        curr_mask = np.array(roi)[0,:,:]
    ...
    if manual==True:  
        return BG,tag_list, dataset,clusters, all_clusters_image 
    else:
...
def plot_roi_traces(rois,analysis_type,summary_save_dir):
    ...
    try:
        os.mkdir(summary_save_dir + 'mean_raw_traces')
    except:
        pass
    finally:
        pass
    ...
    old=glob.glob(summary_save_dir + 'mean_raw_traces'+'\\*.png')
    for entry in old:
        os.remove(entry)

    if analysis_type[0]=='8D_edges_find_rois_save':
        ROI_mod.plot_traces_edges(rois,summary_save_dir + 'mean_raw_traces')
    else:
...
def filter_overlaps_auto_ROIs_with_quantification(quant_to_treshold,rois,save_dir=None,save_fig=True):
    ...
    while len(np.where(all_pre_selected_mask>1)[0]) != 0:
        ...
        for iroi,O_roi in enumerate(rois_in_overlap):
            ...
            evaluated_property[iroi]=(getattr(current_roi,quant_to_treshold[0])[0])
    ...
    values = np.array(list(map(lambda roi : roi.__dict__[quant_to_treshold[0]][0], eliminated_rois)))
    ...
    # pass ROIs through and additional reliability filter.
    rois_to_erase=[]
...
def extract_im_background_forclustering(non_cropped_stack,cropped_stack):
    ...
    def rolling_otsus(window_size,stack):
        ...
        return image>0
    mean_image=non_cropped_stack.mean(0)
    mean_image_cropped=cropped_stack.mean(0)
    stdd=cropped_stack.std(0)
    ...
    otsu_threshold_Value=filters.threshold_otsu(gaussian_filtered)
    ...
    otsu_threshold_Value=filters.threshold_otsu(gaussian_filtered)
    mean_image_otsu_background=np.where(mean_image>otsu_threshold_Value,0,mean_image) 
    ...
    return foreground_otsus, background
...
def produce_feature_table_forClustering(stack,foreground,response_treshold,savedir,categories=None,cat_names=None,min_CSI=0.2,stimulus_information=None, imaging_information=None,apply_CSI_treshold=True,manually_clasiffy_layers=True,apply_otsus=True):
    ...
    #responses_per_epoch_on=[]
    #responses_per_epoch_off
    
    ### temporal monkey patch for miris data TODO: make a 'find cardinal directions' function
    #cardinal_slices=np.array([1,3,5,7]) # separation is probably better if all epochs are included
    #cardinal_slices=np.array(wholeTraces_allTrials_video.keys())
    #valid_epochs=np.where(np.array(stimulus_information['stimtype'])=='driftingstripe')[0]
    #cardinal_slices=cardinal_slices[valid_epochs]
    
    ## find the correct cardinal slices 
    ...
    if manually_clasiffy_layers:
        pref_dir_mat=np.zeros(image_size)
        for label,mask in zip(cat_names,categories):
            if label[0] == 'LPA':
                pref_dir_mat[mask]= np.where(angles==0.0)[0][0] + 1
            elif label[0] == 'LPB':
                pref_dir_mat[mask]= np.where(angles==180.0)[0][0] + 1
            elif label[0] == 'LPC':
                pref_dir_mat[mask]= np.where(angles==90.0)[0][0] + 1
            elif label[0] == 'LPD':
                pref_dir_mat[mask]= np.where(angles==270.0)[0][0] + 1
            else:
                raise Exception ('unlabeled category')
    ##TODO if manually assign layers, then get the labels of the categories and pair them with the correct epoch        
    ##  90, 0, 270, 180 up, right, down, left LPC,LPA,LPD,LPB
    ...
    responsive_pixels=np.zeros(image_size,dtype=int)
    ...
    grid=plt.GridSpec(1, 2, wspace=0.1, hspace=0.2)
    plt.subplot(grid[0])
    plt.imshow(pref_dir_mat_visual*second_foreground)
    plt.title('pixesl wo response filtering')
    plt.subplot(grid[1])
    plt.title('pixels with response filtering')
    plt.imshow(pref_dir_mat_visual*second_foreground*responsive_pixels)
    ...
    plt.imshow(CS_mat2)
    plt.colorbar()
    plt.imshow(np.where(gaussian_filtered>second_treshold,1,0),cmap='gray_r',alpha=0.6)
    plt.title('Otsus foreground from absolute diference between polarity maxima')
    plt.figure()
    plt.imshow(CS_mat2)
    plt.imshow(foreground,cmap='gray_r',alpha=0.6)
    ...
    if apply_CSI_treshold:
        ...
        plt.imshow(CSI_tresholded_mask_on*second_foreground)
        plt.title('ON pixels CSI>%f %f pixels' %(min_CSI,np.sum(CSI_tresholded_mask_on*second_foreground)))
        plt.subplot(grid[1,0])
        plt.imshow(CSI_tresholded_mask_off*second_foreground)
        plt.title('OFF pixels CSI>%f %f pixels' %(min_CSI,np.sum(CSI_tresholded_mask_off*second_foreground)))
        #TODO save figures
    # # # else:
    ...
    # # #plt.subplot(grid[2])
    plt.subplot(grid[0])    
    plt.imshow((pref_dir_mat)*second_foreground)#*second_foreground*(CSI_tresholded_mask_on+CSI_tresholded_mask_off))
    plt.title('pref_dir_calculated_from begining')
    plt.subplot(grid[1])    
    plt.imshow((pref_dir_mat)*second_foreground*CSI_tresholded_mask_on)#*second_foreground*(CSI_tresholded_mask_on+CSI_tresholded_mask_off))
    plt.title('prefdir ON CSI_treshold %f %s'%(min_CSI,apply_CSI_treshold))     
    plt.subplot(grid[2])    
    plt.imshow((pref_dir_mat)*second_foreground*CSI_tresholded_mask_off)#*second_foreground*(CSI_tresholded_mask_on+CSI_tresholded_mask_off))
    ...
    number_of_valid_pixels_on=np.sum(second_foreground*CSI_tresholded_mask_on*responsive_pixels)
    location_on=np.where(second_foreground*CSI_tresholded_mask_on*responsive_pixels)
    location_off=np.where(second_foreground*CSI_tresholded_mask_off*responsive_pixels)
    # check np.unique prefdirmat
    pix_index_1_on=np.zeros(number_of_valid_pixels_on,dtype=int)
    ...
    for ixx in range(1,number_of_epochs+1):
        ...
        plt.imshow(timing_max_off[ixx-1,:,:]*CSI_tresholded_mask_on*second_foreground)
    plt.figure()
    ...
    number_of_valid_pixels_off=np.sum(second_foreground*CSI_tresholded_mask_off*responsive_pixels)
    ...
    for ixx in range(1,number_of_epochs+1):
        plt.subplot(grid[ixx])
        plt.imshow(timing_max_off[ixx-1,:,:]*CSI_tresholded_mask_off*second_foreground)
    plt.figure()
    plt.imshow(test_fig2)
    ...
    features={'on':feature_dataframe_on,'off':feature_dataframe_off}
    ...
    plt.title('CS_mat2')
    plt.axis('off')
    save_str=savedir+'\\clustering_features_edges_stim.pdf'
...
def find_clusters_STICA_JF_BG(cluster_dataset, area_min, area_max,cycle_01_lenght,components=45,CS_filter=None):
    """ Makes pixel-wise plot of DSI
    ...
    """   
    try:
        inclusion_zone=np.array(cluster_dataset.ROIs['I_Zone'][0]) # juan edit. this is a nparray with a mask that corresponds to the neuropile
                                                                   # every ROI outside this zone will be eliminated
    except:
        inclusion_zone=np.array(cluster_dataset.ROIs['I_zone'][0])
    finally:
        pass
    if CS_filter is not None:
        inclusion_zone=inclusion_zone*CS_filter
    ...
    segmentation_approach.append(sima.segment.SparseROIsFromMasks(
            min_size=area_min,smooth_size=0))
    inclusion_filter=sima.segment.ROIFilter(lambda roi: np.sum(np.squeeze(np.array(roi))*np.squeeze(inclusion_zone))==np.sum(np.squeeze(np.array(roi))))
    size_filter = sima.segment.ROIFilter(lambda roi: roi.size >= area_min and \
                                         roi.size <= area_max)
...
def import_cat_ROIs(dataset,mot_corr=True,ROIs_label='categories',ignore_background=True):
    """
    takes ROIs selected with the sima ROIbuddy (Kaifosh et al., 2014) and  
    returns a list of category ROIs (for example, background and inclusion masks)
    and a list of category names correcponding to the category ROIs 
...
def run_cluster_analysis_MH(dataDir,fig_dir,stack,stimulus_information,imaging_information,categories,category_names,I_zone,response_treshold=0,manually_clasiffy_layers=True,apply_otsus=True,clustering_mode='timing&contrast'):
    ...
    lower_size_limit=int(round(0.5/pixel_area)) # the limits are hardcoded here (the units are um^2). they were chosen by Miri based on confocal measurements
    #lower_size_limit=5
    ...
    higher_size_limit=int(round(6.25/pixel_area)) # these limits are within the range calculated by Maisak... ()
    ...
    categories,I_zone,cropped_stack=crop_stack(I_zone,categories,norm_stack)
    
    
    foreground,background=extract_im_background_forclustering(norm_stack,cropped_stack)
    #normalize stack if needed
    ...
    save_path=fig_dir
    features,metadata=produce_feature_table_forClustering(cropped_stack,foreground,response_treshold,save_path,categories=categories,cat_names=category_names,min_CSI=0.2,stimulus_information= stimulus_information, imaging_information=imaging_information,manually_clasiffy_layers=manually_clasiffy_layers,apply_otsus=apply_otsus)
    clusters, all_rois_image,polarity_list =hierarchical_clustering_analysis(norm_stack,foreground.shape,features,categories,lower_size_limit,higher_size_limit,use_pref_all_info=True,saving_path=save_path,clustering_mode=clustering_mode)
    metadata.update({'min_size_pixels':lower_size_limit,'max_size_pixels':higher_size_limit})#,'max_clusters_Hierchachical_clustering':max_clusters})
    print(metadata)
    return clusters, all_rois_image, polarity_list, background, metadata

def hierarchical_clustering_analysis(stack,imsize,features,categories,lower_size_limit,higher_size_limit,use_pref_all_info=True,saving_path=None,CSI_filtered=False,clustering_mode='timing&contrast'):
        ...
        for layer in np.unique(feature_df['pref_dir']):
            ...
            for ix,i in enumerate(range(1,max_cluster_number+1)):
                ...
                for clust_index,indiv_cluster in enumerate(cluster_number):
                    ...
                    index_tuples=zip(cluster_subset[:,0],cluster_subset[:,1])
                    maximum_distance=np.nanmax(cdist(index_tuples,index_tuples,'euclidean'))
                    #decide if the actual cluster is not too big 
                    #(this aproximates the axon terminal to a circle. threshold is 2*root(area/pi. the diameter of a circle with the maximum area allowed)
                    if maximum_distance<np.sqrt(2*np.sqrt(higher_size_limit/np.pi)):
            ...
            for clust_index,indiv_cluster in enumerate(cluster_number):
                ...
                index_tuples=zip(cluster_subset[:,0],cluster_subset[:,1])
                maximum_distance=np.nanmax(cdist(index_tuples,index_tuples,'euclidean'))
                if maximum_distance<np.sqrt(2*np.sqrt(higher_size_limit/np.pi)): #higher_size_limit
            ...
            # if there are no valid clusters, continue to the next layer
...
#                all_rois_image[0:lower_size_limit,1]=0
    ...
    return roi_list, all_rois_image, polarity_list#optimal_maxnumber_of_clusters[-1]
