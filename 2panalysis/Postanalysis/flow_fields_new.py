...
# %%
results_save_dir = r'C:\Users\ptrimbak\Work_PT\2p_data\240701_upd_wt_perspective\control_plots'
...
datapath_0deg = r'C:\Users\ptrimbak\Work_PT\2p_data\240701_upd_wt_perspective\control_wt\0deg' #r'C:\Users\ptrimbak\Work_PT\2p_data\240701_upd_wt_perspective\upd_exp\0deg'
datapath_75deg = r'C:\Users\ptrimbak\Work_PT\2p_data\240701_upd_wt_perspective\control_Wt\75deg'
genotype = 'heterozygous_control'#heterozygous_control  upd_mutant
sparseness_treshold = 0.3
...
def iterate_and_give_rot_vectors(dataframe, pd_name, rotation):
    ...
    for idx in range(len(dataframe)):
        ...
        if dataframe['rotations'].iloc[idx] == rotation:
            coords_x[idx] = RF_coord[0]
            coords_y[idx] = RF_coord[1]
...
def plot_RFonScreen(roi_dataframe,savedir,z_bin=None,rotation=[],filter_polarity=True,treshold=None, cats_to_exclude = []):
    ...
    for pair in treatment_polarity_pairs:
        ...
        for rota in rotation:
            ...
            coords_x = add_jitter(coords_x, jitter=5)
            coords_y = add_jitter(coords_y, jitter=5)
            if sparseness_treshold is not None:
                coords_x = coords_x[:int((len(coords_x)*sparseness_treshold))]
                coords_y = coords_y[:int((len(coords_y)*sparseness_treshold))]
                direction = direction[:int((len(direction)*sparseness_treshold))]
            ...
            if rota == rotation[0]:
                ax1.quiver(coords_x-60, coords_y-80, u , v, scale = 0.5, scale_units='xy', alpha=0.95, color = colors[0], width=widths, cmap='tab10')
            elif rota == rotation[1]:
                ax1.quiver(coords_x-60+rotation[1], coords_y-80, u , v, scale = 0.5, scale_units='xy', alpha=0.5, color = colors[1], width=widths)
            elif rota == rotation[2]:
                ax1.quiver(coords_x-60+rotation[2], coords_y-80, u , v, scale = 0.5, scale_units='xy', alpha=0.5, color = colors[2], width=widths)
            elif rota == rotation[3]:
                ax1.quiver(coords_x-60+rotation[3], coords_y-80, u , v, scale = 0.5, scale_units='xy', alpha=0.5, color = colors[3], width=widths)
            elif rota == rotation[4]:
                ax1.quiver(coords_x-60+rotation[4], coords_y-80, u , v, scale = 0.5, scale_units='xy', alpha=0.5, color = colors[4], width=widths)
            elif rota == rotation[5]:
                ax1.quiver(coords_x-60+rotation[5], coords_y-80, u , v, scale = 0.5, scale_units='xy', alpha=0.5, color = colors[5], width=widths)
            elif rota == rotation[6]:
                ax1.quiver(coords_x-60+rotation[6], coords_y-80, u , v, scale = 0.5, scale_units='xy', alpha=0.5, color = colors[6], width=widths)    
    ...
    if z_bin is not None:
        base_str= savedir +'\\results RFonScreen plot Z%s rel_tresh %.3f _cats %s'% (treshold,cats_to_exclude) 
    else:
        base_str= savedir + '\\results RFonScreen plot allZ rel_tresh %s _cats %s'% (treshold,cats_to_exclude) 
...
# %%
datasets_to_load = []
for root, dirs, files in os.walk(datapath_75deg):
    for file in files:
        if 'dir.pickle' in file:
            datasets_to_load.append(os.path.join(root, file))
...
# upd6on = make_genotype_df(data, genotype='Upd_6f', CS_str='ON')
# upd6off = make_genotype_df(data, genotype='Upd_6f', CS_str='OFF')
control_75deg = make_genotype_df(data, genotype=genotype, CS_str=None, reliability_treshold=reliability_treshold)

# %%
control_0deg['rotations']=0
control_75deg['rotations']=75

combined_df = pd.concat([control_0deg, control_75deg])
...
plot_RFonScreen(combined_df, results_save_dir,z_bin = z_bin,rotation = rotation,filter_polarity = filter_polarities,treshold = reliability_treshold, cats_to_exclude = cats_to_exclude)
