...
results_save_dir = r'U:\Work_PT\Plots\upd_flies_multi_depth\gradient_depths\75deg\C'
...
datapath = r'C:\Users\ptrimbak\Work_PT\2p_data\240531_L2_linear_filters\L2_datset1\processed'
...
def make_fig_base(arr, roi_counts):
    ...
    ax.plot(range(len(mean_r)),mean_r,lw=3, label = f"N = {len(np.unique(L2_df['FlyId']))}(rois: {np.sum(roi_counts)})", marker = 'o')
    ax.legend()
    return fig
