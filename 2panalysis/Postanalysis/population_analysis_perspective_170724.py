...
results_save_dir = r'C:\Users\ptrimbak\Work_PT\2p_data\240701_upd_wt_perspective\All_plots\upd\75deg'
filter_polarities = False
z_bin = None
reliability_treshold = 0.5
CS_filter = [True]
rotation = [None, None, None, None, None, 75, None]
cats_to_exclude=['C', 'A', 'B']
datapath = r'C:\Users\ptrimbak\Work_PT\2p_data\240701_upd_wt_perspective\control_wt'
genotype = 'upd_mutant' #Upd;Gcamp6f/Cyo' heterozygous_control upd_mutant
...
upd6 = make_genotype_df(data, genotype=genotype, reliability_treshold = reliability_treshold, CS_str=None)

# upd6['treatment'] = 'Upd;Gcamp6f/Cyo' heterozygous_control upd_mutant
...
def plot_roivectors(roi_dataframe,savedir,z_bin=None,rotation=None,filter_polarity=True,treshold=None, cats_to_exclude= None):
    ...
    for pair in treatment_polarity_pairs:
        ...
        number_of_rois = np.sum(np.unique(roi_dataframe['roi_count']))
...
def plot_circular_histogram(roi_dataframe,savedir,z_bin=None,rotation=None,filter_polarity=False,treshold=None, cats_to_exclude=None, save_name = ""):
    ...
    if z_bin is not None:
        ...
        base_str= savedir + '\\results circ_hist allZ%s rel_tresh %.3f'% (save_name,treshold )
    save_str1=base_str +'.png'
    plt.title('vector plots reliability_treshold: %.2f'% treshold)
...
def plot_RFonScreen(roi_dataframe,savedir,z_bin=None,rotation=[],filter_polarity=True,treshold=None, cats_to_exclude = []):
    ...
    for pair in treatment_polarity_pairs:
        ...
        # widths =0.002
        my_colors = iter([plt.cm.summer(i) for i in range(8)])
        ...
        for rota in rotation:
            ...
            if rota == rotation[0]:
                ax1.quiver(coords_x-80, coords_y-80, u , v, scale = 0.3, scale_units='xy', alpha=0.95, color = colors[0], width=widths, cmap='summer')
            elif rota == rotation[1]:
                ax1.quiver(coords_x-80+rotation[1], coords_y-80, u , v, scale = 0.3, scale_units='xy', alpha=0.5, color = colors[1], width=widths)
            elif rota == rotation[2]:
                ax1.quiver(coords_x-80+rotation[2], coords_y-80, u , v, scale = 0.3, scale_units='xy', alpha=0.5, color = colors[2], width=widths)
            elif rota == rotation[3]:
                ax1.quiver(coords_x-80+rotation[3], coords_y-80, u , v, scale = 0.3, scale_units='xy', alpha=0.5, color = colors[3], width=widths)
            elif rota == rotation[4]:
                ax1.quiver(coords_x-80+rotation[4], coords_y-80, u , v, scale = 0.3, scale_units='xy', alpha=0.5, color = colors[4], width=widths)
            elif rota == rotation[5]:
                ax1.quiver(coords_x-80+rotation[5], coords_y-80, u , v, scale = 0.3, scale_units='xy', alpha=0.5, color = colors[5], width=widths)
            elif rota == rotation[6]:
                ax1.quiver(coords_x-80+rotation[6], coords_y-80, u , v, scale = 0.3, scale_units='xy', alpha=0.5, color = colors[6], width=widths)    
...
plot_RFonScreen(upd6, results_save_dir,z_bin = z_bin,rotation = rotation,filter_polarity = filter_polarities,treshold = reliability_treshold, cats_to_exclude = ['B','C','D']) # plot layer A
plot_RFonScreen(upd6, results_save_dir,z_bin = z_bin,rotation = rotation,filter_polarity = filter_polarities,treshold = reliability_treshold, cats_to_exclude = ['A','C','D']) # plot layer B
plot_RFonScreen(upd6, results_save_dir,z_bin = z_bin,rotation = rotation,filter_polarity = filter_polarities,treshold = reliability_treshold, cats_to_exclude = ['B','A','D']) # plot layer C
plot_RFonScreen(upd6, results_save_dir,z_bin = z_bin,rotation = rotation,filter_polarity = filter_polarities,treshold = reliability_treshold, cats_to_exclude = ['B','C','A']) # plot layer D
'a'