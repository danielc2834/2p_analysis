...
import traceback
from turtle import color, st, up
import numpy as np
...
class ROI_bg: 
    ...
    def calculateDf(self,stimulus_information,framerate,method='mean',moving_avg = False, bins = 4,bg=None):
        try:
            self.raw_trace
        except NameError:
    ...
    def calculate_freq_powers(self,per_epoch=True):
        ...
        if self.rejected==False:
            self.independent_var_vals = np.where(np.array(self.stim_info['stimtype'])=='G',
                        ...
                        np.nan)
            self.independent_var='frequency'
...
def map_RF_adjust_edge_time(rois,save_path,edges=True,delay_degrees=9.6,delay_use=False,
    ...
    for i, roi in enumerate(rois):
        ...
        for epoch_idx, epoch_type in enumerate(roi.stim_info['stimtype']):
            ...
            if epoch_type == 'ADS' or epoch_type== 'driftingstripe' or epoch_type=='G':
                ...
                if edges and roi.stim_info['polarity'][epoch_idx]==1:#'ONEDGEFIRST' not in roi.stim_info['stim_name']:
                    if roi.CS =='OFF':
                ...
                elif tau_frames>0 and epoch_type=='ADS': # JF edit
                    if roi.CS =='OFF':
    ...
    value_dict={'LPA':1,'LPB':2,'LPC':3,'LPD':4}
    for roi in rois:
        if roi.category!= ['LPA'] and roi.category!= ['LPB'] and roi.category!= ['LPC'] and roi.category!= ['LPD']:
            continue 
        if roi.CS=='ON':
            centers_on[roi.RF_center_coords[0]-3:roi.RF_center_coords[0]+3,roi.RF_center_coords[1]-3:roi.RF_center_coords[1]+3,roi.max_resp_idx_ON]=value_dict[roi.category[0]]
            centers_count_ON[roi.max_resp_idx_ON]+=1
        else:    
            centers_off[roi.RF_center_coords[0]-3:roi.RF_center_coords[0]+3,roi.RF_center_coords[1]-3:roi.RF_center_coords[1]+3,roi.max_resp_idx_OFF]=value_dict[roi.category[0]]
...
def interpolate_signal(signal, sampling_rate, int_rate,stim_duration = 10):
    """
    """
    ...
    timeV=  np.linspace(period,(len(signal)+1)*period,num=len(signal))#(len(signal)+1)*period
    ...
    new_time=np.linspace(1/float(int_rate),stim_duration,steps) #careful if you change int_rate. slightly hardcoded line
...
def reverse_correlation_analysis_JF(rois,stimpath,cf=4,highPassfiltering=True,
    ...
    if hold_out_prediction: 
        ...
        #trace = copy.deepcopy(roi.strf_trace)[:-roi.held_out_frames]
    ...
    for ix,roi in enumerate(rois):
        ...
        roi.STRF_data['strf'], roi.STRF_data['mean_strf'] = apply_reverse_correlation(trace,roi,stim_up_rate,stim_dataframe,snippet,initial_frame,stimulus=stimulus,png_list=png_list,test_indices=roi.STRF_data['test_indices'])#indices=response_idxs
        _,roi.STRF_data['z_score'],_ = calculate_Zscore_STRF(roi,control=False)
        #STRF_response_prediction(roi,trace,stim_up_rate,stim_dataframe,initial_frame,stimulus=stimulus,t_window=snippet,n_epochs=1,held_out_frames = roi.STRF_data['test_indices'])
        ...
        for ix2,i in enumerate(range(20,roi.STRF_data['strf'].shape[0])):
    ...
    # stamin=np.min(stamin)
        ...
        im = ax.imshow(np.mean(roi.STRF_data['strf'][-half_sec:-1,:,:],axis=0),cmap='PRGn',vmax=np.max(roi.STRF_data['strf']),vmin=np.min(roi.STRF_data['strf']))
        ...
        im = ax1.imshow(np.mean(roi.STRF_data['strf'][-half_sec*2:-half_sec,:,:],axis=0),cmap='PRGn',vmax=np.max(roi.STRF_data['strf']),vmin=np.min(roi.STRF_data['strf']))
        ax1.title.set_text('2ndlast.5sec_%s_Z%s'%(roi.category[0], roi.STRF_data['z_score']))
        cbar=fig.colorbar(im, ax=ax1)
        
        ax2 = fig.add_subplot(gs[2])
        im = ax2.imshow(np.mean(roi.STRF_data['strf'][:half_sec,:,:],axis=0),cmap='PRGn',vmax=np.max(roi.STRF_data['strf']),vmin=np.min(roi.STRF_data['strf']))
...
def calculate_STRF_prediction_distributions(rois):
    
    # extract the values required (Z_scores)
...
def apply_reverse_correlation(trace,roi,stim_up_rate,stim_dataframe,snippet,initial_frame,stimulus=None,indices=None,png_list=None,test_indices=np.array([np.nan])):
    ...
    if len(trace_indexes)>0:
        ...
        lost_count = 0
        delay_count = 0
        test_indices = np.array(range(test_indices[0],test_indices[1]))
    ...
    return strf, mean_strf


def STRF_response_prediction(roi,trace,stim_up_rate,stim_dataframe,initial_frame,stimulus=None,t_window=40,n_epochs=1,control=False,held_out_frames=None):
    ...
    if control == False:
        ...
        roi.STRF_data['prediction_corr'] = scipy.stats.pearsonr(roi.STRF_data['strf_prediction'][trace_indexes[0]:trace_indexes[-1]],trace[trace_indexes[0]:trace_indexes[-1]]) 
    else:
...
def plot_psds(rois,save_path,multiple=True):
    ...
    if multiple==True:
        ...
        for ix,epoch in enumerate(epochs):
            ...
            for idx,roi in enumerate(passed_ROIs):
                Temp=roi.signal_psd_dict[epoch]['psd'][ind1]
                Temp=Temp[ind2]
                psds[idx,:]=Temp
                temp2=roi.stim_psd_dict[epoch]['psd'][ind1]
                temp2=temp2[ind2]
                stim_psds[idx,:]=temp2
...
def plot_traces_edges(Rois,save_path):
    ...
    for iroi,roi in enumerate(curr_rois):
        ...
        if np.sum(valid_epochs)>1:   
            ...
            verticalalignment='top', bbox=Textprops)
        try:
            save_str=save_path +'\\mean_raw_traces'+'\\roi_%s %s averaged traces.png' %(iroi,roi.category[0])  
            
        except:
            save_str=save_path+'\\mean_raw_traces'+'\\roi_%s %s averaged traces.png' %(iroi,'no_cat')
        finally:
            pass
