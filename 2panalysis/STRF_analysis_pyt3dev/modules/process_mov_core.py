...
def get_stim_xml_params(t_series_path, stimOuputFile,original_stimDir,Tseries_len,cplusplus=False): #from sebastians code
    """ Gets the required stimulus and imaging parameters.
...
def separate_trials_ROI_v4(time_series,rois,stimulus_information,frameRate, 
    ...
    for iROI, roi in enumerate(rois):
        ...
        roi.calculateDf(stimulus_information,frameRate,method=df_method,moving_avg = mov_av, bins = 3)
        #plt.figure()
        ...
        for iEpoch in trialCoor:        #Juan edit (switched order of loops)
            ...
            if df_use:
                ...
                roi.appendTrace(np.nanmean(respTraces_allTrials_ROIs[iEpoch][iCluster],axis=1),
                                  iEpoch, trace_type = 'response' )
                if np.all(np.isnan(respTraces_allTrials_ROIs[iEpoch][iCluster])):
                    'flaaag'
...
def run_analysis(analysis_params, rois,experiment_conditions,
    ...
    if 'STRF' in analysis_type:
        ...
        map(lambda roi: roi.calculate_reliability(),rois)
        map(lambda roi: roi.calculate_stim_signal_correlation(),rois) 
        map(lambda roi: roi.findMaxResponse_all_epochs(cycle,polarity=expected_polarity), rois) #polarity is relevant for FFF stimuli. if inverted neurons are present or stimulus is different it should be None
        map(lambda roi: roi.setSourceImage(mean_image), rois)
    ...
    if analysis_type == 'A_B_steps':
        ...
        map(lambda roi: roi.calculate_CSI(frameRate=imaging_information['frame_rate']),rois)
        ...
        map (lambda roi: roi.calculate_freq_powers(),rois)
...
def run_ROI_selection(extraction_params,stack,image_to_select=None,**kwargs):
    ...
    if extraction_params['type'] == 'manual':
        ...
        roi_masks, all_rois_image, polarity_list, background, metadata = run_cluster_analysis_MH(dataDir,fig_dir,stack,stimulus_information, imaging_information,categories,category_names,I_zone,clustering_mode=clustering_mode,Tser=Tser)
...
def extract_im_background_forclustering(non_cropped_stack,I_zone):
    ...
    mean_im=non_cropped_stack.mean(0)
...
def produce_feature_table_forClustering(stack,foreground,response_treshold,savedir,categories=None,cat_names=None,min_CSI=0.2,stimulus_information=None, imaging_information=None,apply_CSI_treshold=True,manually_clasiffy_layers=True,apply_otsus=True,orderofedges='off_on',Tser=None):
    ...
    if check==0:
        if ignore_background:
            if time_series is not None:
                _,BG=extract_im_background_forclustering(time_series,time_series)
            else:
                BG=None
        else:
...
def run_cluster_analysis_MH(dataDir,fig_dir,stack,stimulus_information,imaging_information,categories,category_names,I_zone,response_treshold=0,manually_clasiffy_layers=False,apply_otsus=True,clustering_mode='timing&contrast',Tser=None):
    ...
    lower_size_limit=int(round(1/pixel_area)) # the limits (1um^2) are hardcoded here (the units are um^2). they were chosen by Miri based on confocal measurements
    #lower_size_limit=5
    ...
    higher_size_limit=np.ceil(round(6.25/pixel_area)) # these limits are within the range calculated by Maisak... ()
    ...
    clusters, all_rois_image,polarity_list,features_for_clustering =hierarchical_clustering_analysis(stack_subtracted,foreground.shape,features,categories,lower_size_limit,higher_size_limit,use_pref_all_info=True,saving_path=save_path,clustering_mode=clustering_mode)
...
def hierarchical_clustering_analysis(stack,imsize,features,categories,lower_size_limit,higher_size_limit,use_pref_all_info=True,saving_path=None,CSI_filtered=False,clustering_mode='contrast_dif'):
