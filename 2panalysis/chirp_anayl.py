...
def compute_response_qi(trace_list):
    """
    Signal-to-noise ratio across trial repetitions (Baden et al. 2016).
    QI = Var_t[mean_r(C)] / mean_r[Var_t(C)]  where C is T×R matrix.
    Values near 1 = perfectly reproducible; near 0 = pure noise.
    """
    if len(trace_list) < 2:
        return 0.0
    min_len = min(len(t) for t in trace_list)
    C = np.vstack([t[:min_len] for t in trace_list]).T  # T × R
    num = float(np.var(np.mean(C, axis=1)))
    den = float(np.mean(np.var(C, axis=1)))
...
def run_gmm_bic(X, k_range=range(2, 13)):
    ...
    # Log Bayes factors: 2*ΔBIC relative to minimum; >6 = strong evidence
    bic_arr = np.array(bic_vals)
    log_bf  = 2 * (bic_arr - bic_arr.min())
...
def validate_cluster_stability(X, best_k, n_bootstrap=20, frac=0.9):
    """
    Subsamples 90% of ROIs 20×, re-clusters, computes median correlation of
    cluster means with the original clustering (Baden et al. 2016).
    """
    ...
    corrs = []
    n_sub = max(best_k + 1, int(len(X) * frac))
...
def plot_gmm_diagnostics(gmm_diag, dprime, ulbls, bootstrap_corrs, title, save_path):
    """BIC curve, log-Bayes-factors, d′ matrix, bootstrap histogram."""
    has_boot = len(bootstrap_corrs) > 0
    ncols = 4 if has_boot else 3
    fig, axes = plt.subplots(1, ncols, figsize=(5 * ncols, 5))

    ks     = gmm_diag.get('k_range', [])
    bic    = gmm_diag.get('bic', [])
    log_bf = gmm_diag.get('log_bf', [])
    best_k = gmm_diag.get('best_k', None)

    # BIC curve
    ax = axes[0]
    ax.plot(ks, bic, 'o-', color='steelblue', linewidth=2)
    if best_k is not None:
        ax.axvline(best_k, color='red', linestyle='--', label=f'Best k={best_k}')
    ax.set_xlabel('k'); ax.set_ylabel('BIC')
    ax.set_title('BIC (lower = better)', fontweight='bold')
    ax.legend(); ax.grid(True, alpha=0.3)

    # Log Bayes factor
    ax = axes[1]
    ax.bar(ks, log_bf, color='steelblue', alpha=0.7, edgecolor='black')
    ax.axhline(6, color='red', linestyle='--', label='Strong evidence (6)')
    ax.set_xlabel('k'); ax.set_ylabel('2·ΔBIC')
    ax.set_title('Log Bayes Factors', fontweight='bold')
    ax.legend(); ax.grid(True, alpha=0.3)
    ...
    if len(ulbls) > 1 and dprime is not None:
        ...
        ax.set_xticks(range(len(ulbls))); ax.set_xticklabels([f'C{l}' for l in ulbls], fontsize=8)
        ax.set_yticks(range(len(ulbls))); ax.set_yticklabels([f'C{l}' for l in ulbls], fontsize=8)
        plt.colorbar(im, ax=ax, label="d′")
        ax.set_title("d′ separation matrix", fontweight='bold')
        ax.axhline(1.5, color='white', linewidth=0.3, alpha=0.3)
    else:
    ...
    if has_boot:
        ...
        ax.axvline(np.mean(bootstrap_corrs), color='red', linestyle='--',
                   label=f'Mean={np.mean(bootstrap_corrs):.2f}')
        ax.set_xlabel('Median cluster correlation'); ax.set_ylabel('Count')
...
def plot_cluster_heatmap(traces, labels, mean_fps, title, save_path):
    ...
    fig, ax = plt.subplots(figsize=(14, max(4, len(sorted_idx) * 0.12)))
    ...
    for i, (c, b) in enumerate(zip(plot_clusters, boundaries[1:])):
        ...
        ax.text(-0.015 * time[-1], mid, f'C{c}',
                ha='right', va='center', fontsize=9, fontweight='bold',
                color=cmap_c(i % 10), transform=ax.get_yaxis_transform())
    plt.colorbar(im, ax=ax, label='Norm. ΔF/F', shrink=0.6)
    ax.set_xlabel('Time (s)', fontsize=11)
    ax.set_ylabel('ROI (sorted by cluster)', fontsize=11)
    ax.set_title(title, fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def plot_posterior_probabilities(posteriors, labels, title, save_path):
    """Rank-ordered posterior probability curves per cluster (Baden et al. 2016)."""
    unique_clusters = sorted(set(labels))
    plot_clusters   = [c for c in unique_clusters if c != -1]
    if not plot_clusters or posteriors is None:
        return

    fig, ax = plt.subplots(figsize=(10, 5))
    cmap = plt.cm.tab10
    for i, c in enumerate(plot_clusters):
        mask = np.array(labels) == c
        col  = min(c, posteriors.shape[1] - 1)
        post = np.sort(posteriors[mask, col])[::-1]
        x    = np.linspace(0, 1, len(post))
        ax.plot(x, post, color=cmap(i % 10), linewidth=2,
                label=f'C{c} (n={mask.sum()})')
    ax.set_xlabel('Rank (normalised)', fontsize=11)
...
def analyze_response_clustering(condition_rois, fly_rois, segments, mean_fps,
    ...
    for fset in feature_sets:
        ...
        # Bootstrap stability (20 rounds)
        print(f"      Bootstrap validation (k={best_k_gmm}) ...")
        boot_corrs = validate_cluster_stability(X, best_k_gmm)
        ...
        cmeans  = [np.mean([_norm_trace(traces[i][:min_len], mean_fps)
                   ...
                   for c in unique_gmm]
        if len(cmeans) > 1:
            plot_cluster_dendrogram(cmeans,
                title=f'Condition | {fset} | GMM cluster dendrogram (k={best_k_gmm})',
                save_path=os.path.join(output_dir, f'clustering_dendrogram_gmm_{pfx}.png'))

        # Heatmap sorted by GMM cluster
        ...
        plot_posterior_probabilities(posteriors, gmm_labels,
            title=f'Condition | {fset} | GMM posteriors (k={best_k_gmm})',
            save_path=os.path.join(output_dir, f'clustering_posterior_gmm_{pfx}.png'))
    ...
    for fly_id, roi_list in fly_rois.items():
        ...
        for fset in feature_sets:
            ...
            plot_gmm_diagnostics(gmm_diag_f, dp_f, ul_f, [],
            ...
            plot_posterior_probabilities(post_f, gmm_lbl_f,
                title=f'Fly {fly_id} | {fset} | GMM posteriors',
                save_path=os.path.join(output_dir, f'clustering_posterior_gmm_{pfx_f}.png'))
